...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        "*** YOUR CODE HERE ***"
        # def getQVal(state, action, depth):
        #     if self.mdp.isTerminal(state) or depth == 0:
        #         return 0
        #     else:
        #         sumStateProbs = 0
        #         for T in self.mdp.getTransitionStatesAndProbs(state, action):
        #             if self.mdp.isTerminal(T[0]):
        #                 sumStateProbs += 0
        #             else:
        #                 sumStateProbs += T[1]*(self.mdp.getReward(state, action, T[0]) +
        #                                        self.discount*max([getQVal(T[0], aPrime, depth - 1) for aPrime in self.mdp.getPossibleActions(T[0])]))
        #         return sumStateProbs
        temp = {}
        for i in range(self.iterations):
            for s in self.mdp.getStates():
                if self.mdp.isTerminal(s) or s == 'TERMINAL_STATE':
                    continue
                temp[s] = max([self.computeQValueFromValues(s,act) for act in self.mdp.getPossibleActions(s)])
            for k in temp.keys():
                self.values[k] = temp[k]
